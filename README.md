...
The homepage section `#behind-the-craft` (between the keepsake stories and “Something meaningful”) plays four product films from `videos/` (`product-video-01.mp4` … `04.mp4`, each with a matching `-poster.jpg`). Videos are muted, looped and `playsinline`; `script.js` attaches each source only when it comes within 300px of the viewport, plays it only while it is on screen, pauses it off-screen, and offers a pause/play toggle. To replace a film, overwrite the file in `videos/` with the same name.
